    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `/mutualfunds/${category}/${subcategory}.json`
        );
        if (!response.ok) {
      ...
      <section>
        <div className="container">
          <div className="breadcrumb_subcategory">
            ...
            <Link to={`/mutualfunds/${category}`}>
              {" "}
              {category.charAt(0).toUpperCase() + category.slice(1)} Funds
            </Link>{" "}
            &gt;
            {data[subcategory].length ? (
              <span>{data[subcategory][0].subcategory_name}</span>
            ) : (
              ""
            )}
          </div>
